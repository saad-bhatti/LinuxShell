...
/**
 * Represents the command echo in JShell that overwrite input text to files
 *
 */
public class EchoOverwrite extends CommandHandler {

  /**
   * Create an instance variable in ChangeDirectory type
   */
  private ChangeDirectory cd;

  /**
   * Constructs a new EchoOverwrite object. Initializes the cd and assign cd memory as
   * ChangeDirectory
   */
  public EchoOverwrite() {
   ...
   * exists under the current directory, then overwrites the givenText to the filename file
   * 
  ...
  private void storeTextInCurrent(FileSystem fileSystem, String fileName, String givenText) {
    ...
    // File with fileName does exist
      ...
      // Overwrite the existing text in the file with givenText
      super.fileObject.setContent(givenText);
    }
  }
   ...
   * then overwrites the givenText into the givenName file.
   * 
   * @param fileSystem The current file system in the JShell and contains a reference to the root
   *        and current directory.
   ...
   * text in a file under current directory or specified by a path. If the file exists, it
   * overwrites the content. Otherwise, it stores the content in a new file. If successful, it will
   * return "" and returns an error otherwise.
   * 
   * @param fileSystem The current file system in the JShell and contains a reference to the root
   *        and current directory.
